...
constexpr const char* TMP_PATH = "/tmp/awarp.socket";
constexpr const char* VAR_PATH = "/var/run/awarp/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("awarp")) {
    logger.debug() << "/var/run/awarp seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("awarp")) {
    logger.warning() << "Failed to create /var/run/awarp";
    return TMP_PATH;
  }

  if (chmod("/var/run/awarp", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/awarp";
    return TMP_PATH;
  }
